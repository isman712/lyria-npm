...
async function BuscarLetra(cancion) {
    try {
        ...
        const lineas = $('div[jsname="WbKHeb"] span').map((i, el) => {
            return $(el).text(); 
        }).get();

        if(lineas.length > 0) {
            let artistag = $('div.rVusze span:contains("Artista") + span a').text();
            let albumg = $('div[data-attrid="kc:/music/recording_cluster:first album"] a').text();
            let fechag = $('div.rVusze span:contains("Fecha de lanzamiento") + span span').text();
            let generosg = $('div.rVusze span').filter((i, el) => {
                return /Género|Géneros/.test($(el).text());
            }).next('span').find('a').map((i, el) => {
                return $(el).text();
            }).get();
